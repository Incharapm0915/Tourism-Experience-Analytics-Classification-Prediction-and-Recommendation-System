# ============================
# Custom Recommender Classes
# ============================

class HybridRecommender:
    def __init__(self, *args, **kwargs):
        pass
    # Define other methods as per your real class
...
def predict_rating(models, user_features):
    ...
    try:
        ...
        feature_vector = {feature: user_features.get(feature, 0) for feature in feature_names}
...
def show_overview(master_df, models):
    ...
    with col1:
        if 'regression' in models:
            r2 = models['regression']['metadata']['performance']['test_r2']
    ...
    with col2:
        if 'classification' in models:
            f1 = models['classification']['metadata']['performance']['test_f1_macro']
        ...
        with col2:
            ...
            if mode_pred is not None:
                ...
                if mode_probs is not None:
                    ...
                    fig = px.bar(prob_df, x='Mode', y='Probability', title="Visit Mode Probabilities")
                    fig.update_layout(height=300)
                    st.plotly_chart(fig, use_container_width=True)
            else:
                st.error(mode_info)
...
def show_recommendations_page(master_df, models):
    ...
    with col1:
        if 'UserId' in master_df.columns:
            ...
            selected_user = st.selectbox("Select User ID", unique_users[:100])
        else:
    ...
    with col2:
        if 'UserId' in master_df.columns and selected_user in master_df['UserId'].values:
            ...
            if 'AttractionType' in master_df.columns:
                fav_type = user_data['AttractionType'].mode()
                if len(fav_type) > 0:
                    st.write(f"- Favorite attraction type: {fav_type.iloc[0]}")
